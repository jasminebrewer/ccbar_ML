...
//____________________________________________________________________________________________
splitting iterative_reclustering_doubletag(fastjet::PseudoJet& jet, vector<fastjet::PseudoJet>& tagged_list, double jetR, string algorithm) { 
  
  vector< fastjet::PseudoJet > emptySplitting = {fastjet::PseudoJet(), fastjet::PseudoJet(), fastjet::PseudoJet()};

  vector<fastjet::PseudoJet> jet_constituents = jet.constituents();

  fastjet::JetAlgorithm jet_reclustering_algorithm;

  
  if (algorithm=="kt") {
    // set the kt algorithm for jet reclustering       
    jet_reclustering_algorithm = fastjet::kt_algorithm;
  }
  else {
    if (algorithm=="C/A") {
      // set the cambridge algorithm for jet reclustering       
      jet_reclustering_algorithm = fastjet::cambridge_algorithm;
    }
    else cout << "Allowed values of algorithm are 'kt' and 'C/A' and I didn't get either of those. Using the default C/A algorithm for reclustering" << endl;
  }
...
//_________________________________________________________________________________________________
int initialize_pythia( string paramfile_name, Pythia& pythia, struct trackCuts& track_cuts, bool D0decay) {
  ...
  if (!D0decay) pythia.readString(to_string(constants::D0)+":mayDecay = off");
...
//____c/cbar pair). If processMC is true, write the properties of the splitting from MC-level to___
//____the outfile, otherwise writes the properties of thesplitting between the D/Dbar (or c/cbar)__
//____pair found by iterative declustering into outfile____________________________________________
//_________________________________________________________________________________________________
void process_jets(Event& event, vector<fastjet::PseudoJet>& final_particles, vector<fastjet::PseudoJet>& tagged_particles, vector<int>& tagged_particle_ids, ofstream& outfile, struct trackCuts& track_cuts, int iEvent, bool processMC, string algorithm ) {
  ...
  for (int ijet = 0; ijet < njets; ijet++) {
    ...
    splitting tagged_split;

    if ( !processMC ) tagged_split = iterative_reclustering_doubletag( jet, tagged_particles, track_cuts.jetR, algorithm );
    else tagged_split = find_common_splitting(event, tagged_particles[0].user_info<MyUserInfo>().global_index(), tagged_particles[1].user_info<MyUserInfo>().global_index());
...
void cluster_jets(Event& event, vector<fastjet::PseudoJet>& final_particles, vector<fastjet::PseudoJet>& tagged_particles,vector<int>& tagged_particle_ids, ofstream& outfile, struct trackCuts& track_cuts, int iEvent, int flagged ) {
  ...
  for (int ijet = 0; ijet < njets; ijet++) {
    ...
    // if the jet was flagged but the tagged particles are not among the jet constituents (i.e. somewhere else in the event, unflag it
    if ( tagged_particles.size()==2 && flagged==1 && jet_contents_by_pdg( jet, tagged_particle_ids, tagged_particles )!=one_each ) flagged=0;

    // write out to file
    outfile << "E " << iEvent << " " << flagged << endl;
    
    vector<fastjet::PseudoJet> constituents = jet.constituents();
    for (unsigned int i=0; i<constituents.size(); i++) {
      outfile << constituents[i].pt() / jet.pt() << ", " << constituents[i].eta() - jet.eta() << ", " << constituents[i].phi() - jet.phi() << ", " << constituents[i].user_info<MyUserInfo>().pdg_id() << endl;
    }
  }
}
...
splitting find_common_splitting(Event& event, int particle_1, int particle_2) {
  ...
  fastjet::PseudoJet daugh1, daugh2;
  vector<float> kt_list;
  splitting split;

  bool splitting_entered = false;
  ...
  while ( !is_contained( event[ current_mother ].status(), incoming_status ) ) {
    
    current_particle = event[ current_particle ].mother1();
    
    daugh1 = final_jet( event, event[ current_particle ].daughter1() );
    daugh2 = final_jet( event, event[ current_particle ].daughter2() );
    current_mother = event[ current_particle ].mother1();
    
    // if you find the ccbar splitting, keep it
    if ( event[ particle_2 ].isAncestor( current_particle ) && !splitting_entered ) {
      split = { final_jet( event, current_particle ), daugh1, daugh2 };
      splitting_entered = true;
    }
    
    // if ( daugh1.perp() < daugh2.perp()) swap( daugh1, daugh2 );
    kt_list.push_back( get_kt( daugh1, daugh2 ) );
    
  }

  // find the kt of the ccbar in the kt list for the full event, to know the level of the ccbar splitting
  sort( kt_list.begin(), kt_list.end(), greater<float>() );
  int level = 0;
  while ( kt_list[level]-pow(10,-5) > get_kt( split.out1, split.out2 ) ) {
    level++;
  }
	 
  // if ( daugh1.perp() < daugh2.perp()) swap( daugh1, daugh2 );
  
  // splitting split = { final_jet( event, current_particle ), daugh1, daugh2 };
  return split;
}


void depth_first(Event& event, int current_particle, vector<int>& was_visited, vector<fastjet::PseudoJet>& final_MC_particles) {

  if ( is_contained( current_particle, was_visited ) ) return;

  was_visited.push_back( current_particle );
  if ( event[ current_particle ].isFinalPartonLevel() ) final_MC_particles.push_back( as_pseudojet( event[ current_particle ] ) );

  vector<int> daughters = event[ current_particle ].daughterList();
  for (int i=0; i<daughters.size(); i++) {
    depth_first( event, daughters[i], was_visited, final_MC_particles );

  }
}

fastjet::PseudoJet final_jet(Event& event, int current_particle) {

  vector<int> was_visited;
  vector<fastjet::PseudoJet> final_MC_particles;

  depth_first( event, current_particle, was_visited, final_MC_particles );
  
  return cluster_all( final_MC_particles, 1.0 );
}

fastjet::PseudoJet cluster_all( vector<fastjet::PseudoJet> particles, double jetR ) {
  ...
  fastjet::ClusterSequence reclustering(particles, jet_def_recluster);
  fastjet::PseudoJet jet = (reclustering.inclusive_jets(0.0)).at(0);  // get the reclustered jet                                         
